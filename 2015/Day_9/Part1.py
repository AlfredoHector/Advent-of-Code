...
def calculate(city, times, log= []) -> int:
    ...
    for a in possibles:
        ...
        if min_distance == 0:
            min_distance = b
            min_city = a
        elif min_distance > b:         # I am doing this in a rush, but there is a posibility
            min_distance = b           # that you get 2 cities with the minimum distance, wich
            min_city = a               # could mean you get wrong results. if you do, fix here...
    
    return min_distance + calculate(min_city,times,log)
